# cashless_sec_1_2_3_4_5_A_210204 switching to new id
...
#cashless_190403.R switching to 2017 DCSC/SCPC: (id instead of id, "work_oth" instead of "work_other", "income" needs to be converted from categories to numbers)
...
length(unique(subset(d10, amnt <= 100)$id))# num of resp
#
...
length(unique(d10[d10$amnt <= 100, ]$id)) # resp for the above
nrow(d10[d10$amnt > 100 & d10$amnt <= 400, ])# num trans betweem $100 and $400
length(unique(d10[d10$amnt > 100 & d10$amnt <= 400, ]$id)) # resp for the above
...
(all_num_resp = length(unique(d10$id)))# num respondents
...
all_unique_id = d10[!duplicated(d10$id),  ]
dim(all_unique_id)
...
(all_val_trans = sum(d10$amnt, na.rm = T)/length(unique(d10$id)))# value per resp in 3 days
(all_val_trans_w = sum(d10$weight_1_trans*d10$amnt, na.rm = T)/length(unique(d10$id)))# value per resp in 3 days
...
(both_resp = length(unique(both$id)))# num resp
...
(both_val_trans_w = sum(both$weight_1_trans*both$amnt, na.rm = T)/length(unique(both$id)))# value per resp in 3 days
...
(no_cc_resp = length(unique(no_cc$id)))# num resp
...
(no_cc_val_trans_w = sum(no_cc$weight_1_trans*no_cc$amnt, na.rm = T)/length(unique(no_cc$id)))# value per resp in 3 days
...
(no_dc_resp = length(unique(no_dc$id)))# num resp
...
(no_dc_val_trans_w = sum(no_dc$weight_1_trans*no_dc$amnt, na.rm = T)/length(unique(no_dc$id)))# value per resp in 3 days
...
# (no_dc_cc_resp = length(unique(no_dc_cc$id)))# num resp
...
# (no_dc_cc_val_trans_w = sum(no_dc_cc$weight_1_trans*no_dc_cc$amnt, na.rm = T)/length(unique(no_dc_cc$id)))# value per resp in 3 days
...
(none_banked_resp = length(unique(none_banked$id)))# num resp
...
(none_banked_val_trans_w = sum(none_banked$weight_1_trans*none_banked$amnt, na.rm = T)/length(unique(none_banked$id)))# value per resp in 3 days
...
(none_unb_resp = length(unique(none_unb$id)))# num resp
...
(none_unb_val_trans_w = sum(none_unb$weight_1_trans*none_unb$amnt, na.rm = T)/length(unique(none_unb$id)))# value per resp in 3 days
