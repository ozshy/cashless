# cashless_fig_3_5_210204.R replacing id
...
length(unique(d10$id))# num respondents
...
nrow(income99)
nrow(incomeinf)
length(unique(income25$id))
length(unique(income49$id))
length(unique(income74$id))
length(unique(income99$id))
length(unique(incomeinf$id))

frac_cash_all = d10 %>% group_by(id) %>% summarize(mean(pi == "cash"))
head(frac_cash_all)
nrow(frac_cash_all)
#
frac_cash_income25 = income25 %>% group_by(id) %>% summarize(mean(pi == "cash"))
head(frac_cash_income25)
nrow(frac_cash_income25)# num payments
#
frac_cash_income49 = income49 %>% group_by(id) %>% summarize(mean(pi == "cash"))
head(frac_cash_income49)
nrow(frac_cash_income49)# num payments
#
frac_cash_income74 = income74 %>% group_by(id) %>% summarize(mean(pi == "cash"))
head(frac_cash_income74)
nrow(frac_cash_income74)# num payments
#
frac_cash_income99 = income99 %>% group_by(id) %>% summarize(mean(pi == "cash"))
head(frac_cash_income99)
nrow(frac_cash_income99)# num payments
#
frac_cash_incomeinf = incomeinf %>% group_by(id) %>% summarize(mean(pi == "cash"))
...
(all_median_age = median(all_unique_id$age, na.rm = T))# median age
#
income25_unique_id = income25[!duplicated(income25$id),  ]
...
#
income49_unique_id = income49[!duplicated(income49$id),  ]
...
#
income74_unique_id = income74[!duplicated(income74$id),  ]
...
#
income99_unique_id = income99[!duplicated(income99$id),  ]
...
#
incomeinf_unique_id = incomeinf[!duplicated(incomeinf$id),  ]
